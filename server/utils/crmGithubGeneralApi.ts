...
const CRM_GITHUB_SYNC_CONTEXT_LEGACY_ENDPOINT =
  'crm/general/github/sync/context'
const CRM_CACHE_PREFIX = 'crm'

const CRM_GITHUB_TTL_BY_PATH: Array<{ pattern: RegExp; ttl: number }> = [
  {
    pattern:
      /^projects\/[^/]+\/github\/(commits|branches|pull-requests|collaborators)(\/|$)/,
    ttl: 60,
  },
...
function resolveCrmGithubGetTtl(path: string) {
  const normalizedPath = path.replace(/^\/+/, '')
  const override = CRM_GITHUB_TTL_BY_PATH.find(({ pattern }) =>
