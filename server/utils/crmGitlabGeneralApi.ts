...
const CRM_GITLAB_SYNC_CONTEXT_LEGACY_ENDPOINT = 'crm/general/gitlab/sync/context'
const CRM_CACHE_PREFIX = 'crm'

const CRM_GITLAB_TTL_BY_PATH: Array<{ pattern: RegExp; ttl: number }> = [
  { pattern: /^projects\/[^/]+\/gitlab\/(commits|branches|pull-requests|collaborators)(\/|$)/, ttl: 60 },
...
function resolveCrmGitlabGetTtl(path: string) {
  const normalizedPath = path.replace(/^\/+/, '')
  const override = CRM_GITLAB_TTL_BY_PATH.find(({ pattern }) => pattern.test(normalizedPath))
