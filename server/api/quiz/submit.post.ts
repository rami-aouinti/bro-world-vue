...
export default defineEventHandler(async (event): Promise<QuizSubmitApiResponse> => {
  const body = await readBody<QuizSubmitPayload>(event)

  return mutatingPrivateApiCall<QuizSubmitApiResponse>(event, '/quiz/general/submit', {
