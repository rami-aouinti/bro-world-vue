...
export default defineEventHandler(async (event) => {
  ...
  if (typeof query.categoryId === 'string') {
    products = products.filter(product => product.categoryIds.includes(query.categoryId as string))
  }
  ...
  if (typeof query.search === 'string' && query.search.trim()) {
    ...
    products = products.filter(product => product.name.toLowerCase().includes(q) || product.description.toLowerCase().includes(q))
  }

  if (typeof query.brand === 'string') {
    products = products.filter(product => product.attributes.brand === query.brand)
  }

  if (typeof query.material === 'string') {
    products = products.filter(product => product.attributes.material === query.material)
  }

  if (typeof query.color === 'string') {
    products = products.filter(product => product.variants.some(variant => variant.color === query.color))
  }

  if (typeof query.size === 'string') {
    products = products.filter(product => product.variants.some(variant => variant.size === query.size))
  }
