...
export default defineEventHandler(
  async (event): Promise<CrmJsonReportResponse | string> => {
    const format = (
      getQuery(event).format as string | undefined
    )?.toLowerCase() as CrmReportFormat | undefined
    ...
    const report = await cachedCrmGeneralGet<CrmJsonReportResponse | string>(
      event,
      'reports',
      {
        format,
