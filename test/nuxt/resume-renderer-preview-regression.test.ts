...
describe('ResumeRenderer preview regression coverage', () => {
  it('shows the main preview on first render with header + main + aside content', async () => {
    const wrapper = await mountSuspended(ResumeRenderer, {
      props: {
        ...
        templateSkin: RESUME_TEMPLATE_SKINS['cv-classic'],
      },
    })
      ...
      for (const skinId of RESUME_SKIN_IDS) {
        ...
        const wrapper = await mountSuspended(ResumeRenderer, {
          props: {
            ...
            designState: { layoutMode: layout.structure, palette: skinId },
          },
        })
        expect(
          wrapper.find('article.resume-skin').exists(),
          `${layout.layoutId} × ${skinId}: root missing`,
        ).toBe(true)
      }
    }
  }, 120000)
  ...
  it('matches premium curated template snapshots', async () => {
    const premiumTemplates = RESUME_TEMPLATES_CATALOG.filter(
      (template) =>
        template.type === 'resume' &&
        template.image !== '/img/cv/resume-modern.svg',
    )
    ...
    for (const template of premiumTemplates) {
      const wrapper = await mountSuspended(ResumeRenderer, {
        props: {
          ...
          templateSkin: RESUME_TEMPLATE_SKINS['cv-socle'],
          designState: {
            layoutMode: template.structureId,
            palette: template.skinId,
          },
        },
      })
