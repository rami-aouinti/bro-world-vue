...
import { ALL_RESUME_DOCUMENT_TEMPLATES_CATALOG, RESUME_LAYOUTS_CATALOG } from '~/constants/resumeTemplates.catalog'
...
describe('resume layouts integrity', () => {
  ...
  it('manual catalog templates reference an existing preview image in public/img/cv', () => {
    const manualTemplates = ALL_RESUME_DOCUMENT_TEMPLATES_CATALOG.filter((template) => !template.id.includes('layout-'))

    for (const template of manualTemplates) {
      const diskPath = resolve(process.cwd(), `public${template.image}`)
      expect(existsSync(diskPath), `${template.id}: missing image ${template.image}`).toBe(true)
    }
  })
  ...
  it('catalog layouts keep one unique ordered entry per section key', () => {
    for (const layout of RESUME_LAYOUTS_CATALOG) {
      ...
      const orderByZone = layout.sections.reduce<Record<'main' | 'aside', number[]>>((acc, section) => {
        acc[section.zone].push(section.order)
        return acc
      }, { main: [], aside: [] })

      expect(orderByZone.main).toEqual([...orderByZone.main].sort((a, b) => a - b))
      expect(orderByZone.aside).toEqual([...orderByZone.aside].sort((a, b) => a - b))
    }
  })
})
