...
interface Props {
  ...
  centerCards?: string[]
  centerMelds?: string[][]
  meldsByPlayer?: Partial<
    Record<'player' | 'aiTop' | 'aiRight' | 'aiLeft', string[][]>
  >
...
const meldsBySeat = computed(() =>
  playersWithSeats.value.reduce<Record<string, string[][]>>(
    (accumulator, player) => {
      accumulator[player.seat] =
        props.meldsByPlayer[player.id as keyof typeof props.meldsByPlayer] ?? []
      return accumulator
    },
    {},
  ),
...
const hasCenterContent = computed(
  () => props.centerCards.length > 0 || props.centerMelds.length > 0,
)
    ...
    <template #surface>
      ...
      >
        <div
          ...
          :class="{
            'seat-melds__group--side':
              player.seat === 'east' || player.seat === 'west',
        ...
        >
          ...
          >
            <span class="meld-card__corner"
              >{{ cardParts.rank }}{{ cardParts.suit }}</span
            >
            <span class="meld-card__center">{{ cardParts.suit }}</span>
            <span class="meld-card__corner meld-card__corner--bottom"
              >{{ cardParts.rank }}{{ cardParts.suit }}</span
            >
          </span>
        </div>
      ...
      <slot name="center">
        <div v-if="hasCenterContent" class="center-fallback">
          <div v-if="centerCards.length" class="center-fallback__row">
            <span
              ...
              class="center-fallback__card"
              >{{ card }}</span
            >
          </div>
          <div v-if="centerMelds.length" class="center-fallback__column">
            <div
              v-for="(meld, meldIndex) in centerMelds"
              :key="`center-meld-${meldIndex}`"
              class="center-fallback__row"
            >
              <span
                v-for="(card, cardIndex) in meld"
                :key="`center-meld-${meldIndex}-${cardIndex}`"
                class="center-fallback__card"
                >{{ card }}</span
              >
            </div>
        ...
        <p v-else class="text-caption mb-0 text-medium-emphasis">
          {{ t('gameComponents.cardTable.emptyTable') }}
        </p>
      </slot>
    </template>
...
.seat-melds__group--west .meld-card--side {
  transform: none;
}
.meld-card {
  display: inline-flex;
...
.table-theme--uno .meld-card {
  border-color: rgba(146, 64, 14, 0.45);
  background: linear-gradient(165deg, #fff8dc, #fef3c7);
}
.table-theme--belote .meld-card {
  border-color: rgba(30, 64, 175, 0.32);
  background: linear-gradient(165deg, #eff6ff, #dbeafe);
}
.table-theme--poker .meld-card {
...
.table-theme--uno .center-fallback__card {
  background: #fff8dc;
  color: #78350f;
  border: 1px solid rgba(146, 64, 14, 0.25);
}
.table-theme--belote .center-fallback__card {
  background: #eff6ff;
  color: #1e3a8a;
  border: 1px solid rgba(30, 64, 175, 0.3);
}
.table-theme--poker .center-fallback__card {
  background: #fff1f2;
  color: #7f1d1d;
