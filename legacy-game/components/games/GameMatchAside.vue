...
const getAsideSurfaceStyle = (game: GameEntry | null) => {
  ...
  const overlayByStyle: Record<GameVisualStyle, string> = {
    ...
    classic:
      'linear-gradient(140deg, rgba(120,53,15,.12), rgba(251,191,36,.12))',
...
<template>
  ...
  <div
    v-else-if="!isGameStarted && selectedGame.developmentStatus !== 'playable'"
  ...
  >
    ...
    <v-btn
      ...
      @click="
        emit('action', comingSoonActionId(selectedGame.developmentStatus))
      "
    >
  ...
  >
    ...
    <p
      v-if="sanitizedLiveStatus(liveGamePanel)"
      class="text-caption text-medium-emphasis mb-0"
    >
