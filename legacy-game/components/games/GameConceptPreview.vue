...
<template>
  <v-card variant="outlined" class="pa-4 d-flex flex-column ga-4">
    ...
    <div>
      ...
      <ol class="pl-5 mb-0">
        <li v-for="(step, index) in coreLoop" :key="`loop-step-${index}-${step}`" class="mb-1">
          {{ step }}
        </li>
      </ol>
    </div>
    ...
    <div>
      ...
      <ul class="pl-5 mb-0">
        <li v-for="rule in rulesSummary" :key="`rule-${rule}`" class="mb-1">{{ rule }}</li>
      </ul>
    </div>
    ...
    <div>
      ...
      <ul class="pl-5 mb-0">
        <li v-for="section in uxMockSections" :key="`ux-${section}`" class="mb-1">{{ section }}</li>
      </ul>
    </div>
    ...
    <div v-if="monetizationNotes.length">
      ...
      <ul class="pl-5 mb-0">
        <li v-for="note in monetizationNotes" :key="`monetization-${note}`" class="mb-1">{{ note }}</li>
      </ul>
    </div>
    ...
    <div>
      ...
      <ul class="pl-5 mb-0">
        <li v-for="milestone in milestones" :key="`milestone-${milestone}`" class="mb-1">{{ milestone }}</li>
      </ul>
    </div>
  </v-card>
</template>
