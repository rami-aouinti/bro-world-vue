...
const step = (ts: number) => {
  ...
  const dt = lastFrameTs.value === null ? 0.016 : Math.min((ts - lastFrameTs.value) / 1000, 0.05);
  lastFrameTs.value = ts;

  rocketVelocity.value = Math.min(rocketVelocity.value + GRAVITY * dt, MAX_FALL_SPEED);
  ...
  obstacles.value = obstacles.value.filter((obstacle) => obstacle.x + OBSTACLE_WIDTH > -30);
  spawnIfNeeded();
    ...
    >
      ...
      >
        ...
        <div
          ...
          :style="{ top: `${((obstacle.gapTop + OBSTACLE_GAP) / FIELD_HEIGHT) * 100}%` }"
        />
      </div>
      ...
      <div v-if="gameStatus !== 'running'" class="overlay">
        <p class="text-h6 mb-3">
          {{ gameStatus === "idle" ? "Prêt à décoller ?" : score >= WIN_SCORE ? "Mission réussie" : "Crash détecté" }}
        </p>
        <p class="text-body-2 text-medium-emphasis mb-4">
          {{ gameStatus === "idle" ? "Passe entre les obstacles pour scorer." : "Appuie pour repartir en orbite." }}
        </p>
...
.game-field {
  ...
  text-align: left;
  background:
    radial-gradient(circle at 84% 18%, rgba(139, 92, 246, 0.3), transparent 32%),
