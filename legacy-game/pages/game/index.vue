...
const { featuredGames: featuredGamesItems, isLoading: isFeaturedGamesLoading } =
...
const gameStatusLabel = computed(() => {
  ...
  if (isGameStarted.value) {
    const moodLabel = selectedGame.value.mood
      ? moodLabelMap[selectedGame.value.mood]
      : 'session'
...
const selectedGameLevel = computed(() => {
  ...
  const level = (
    selectedGame.value as (GameEntry & { level?: string | number }) | null
  )?.level
  return level ? String(level) : fallbackValue.value
})

const gameDetailTags = computed(
  () => selectedGame.value?.tags?.filter(Boolean) ?? [],
)
...
onMounted(async () => {
  await Promise.all([gameCatalogStore.fetchCatalog(), gamesStore.fetchGames()])
})
  ...
  <PlatformSplitLayout>
    <template #sidebar>
      ...
      <div class="d-flex align-center flex-wrap ga-2 mb-0">
        ...
        <v-chip v-if="gameStatusLabel && selectedCategory">{{
          gameStatusLabel
        }}</v-chip>
        <v-chip
      ...
      <Transition :name="flowTransitionName" mode="out-in">
        <div :key="`aside-${rightPanelState}-${selectedGameId ?? 'none'}`">
          <GameQuickAccessPanel
            ...
            :title="
              te('gamePage.quickAccess.title')
                ? t('gamePage.quickAccess.title')
                : 'Quick Access'
            "
        ...
        <div :key="mainContentTransitionKey">
          ...
          <section v-else-if="catalogError">
            ...
            <v-btn
              color="primary"
              variant="outlined"
              @click="gameCatalogStore.fetchCatalog(true)"
            >
              {{ t('common.retry') }}
            </v-btn>
          </section>
                ...
                >
                  ...
                  <div class="w-100 pa-3">
                    <div
                      ...
                      class="catalog-image"
                      :style="
                        getCatalogImageStyle(subCategory.id, 'subCategory')
                      "
          ...
          <section
            v-else-if="selectedSubCategory && !selectedGame"
            class="mb-4"
          >
            <v-row class="ga-0 ga-md-1">
              <v-col
