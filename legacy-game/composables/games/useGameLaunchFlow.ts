...
import type { StartGameSessionResponse, GameLevel } from "~/composables/api/useGameSessionsApi";
...
interface UseGameLaunchFlowOptions {
  ...
  startGameSession: (gameId: string, level: GameLevel) => Promise<StartGameSessionResponse>;
...
export const useGameLaunchFlow = ({
  ...
  const canLaunchSelectedGame = computed(() => {
    if (!selectedGame.value?.component || !isGameAvailableForLaunch(selectedGame.value)) {
      return false;
    }
