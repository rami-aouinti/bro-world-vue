...
export const useWorldCrmGitlabStore = defineStore('world-crm-gitlab', () => {
  ...
  function withSyncContextQuery(
    ...
    const isSyncContextEndpoint = endpoint.startsWith('/api/world/crm/general/gitlab/sync/context')
  ...
  return {
    ...
    bootstrapSync: (body: CrmGithubBootstrapPayload) =>
      mutate<CrmGithubBootstrapResponse>('/api/world/crm/general/gitlab/sync/bootstrap', 'POST', {
        body,
    ...
    getSyncJobStatus: (jobId: string, options?: { force?: boolean }) =>
      get<CrmGithubSyncJobStatus>(`/api/world/crm/general/gitlab/sync/jobs/${jobId}`, undefined, options).then((response) => {
        rememberSyncContext({
          jobId: response.id,
    ...
    getTaskRequestBranches: (taskRequestId: string, options?: { force?: boolean }) =>
      get<CrmGithubListResponse>(`/api/world/crm/general/task-requests/${taskRequestId}/gitlab/branches`, undefined, options),
    createTaskRequestBranch: (
      taskRequestId: string,
      body: CrmGithubTaskRequestBranchPayload,
    ) =>
      mutate(`/api/world/crm/general/task-requests/${taskRequestId}/gitlab/branches`, 'POST', {
        body,
