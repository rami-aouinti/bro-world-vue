...
function isFresh(lastFetchedAt: number | null) {
  return typeof lastFetchedAt === 'number' && Date.now() - lastFetchedAt < CRM_REFERENCE_TTL_MS
}
...
export const useCrmReferenceOptionsStore = defineStore('crm-reference-options', () => {
  ...
  async function fetchEmployees(force = false) {
    if (!force && isFresh(employeesFetchedAt.value) && employees.value.length) {
  ...
  async function fetchProjects(force = false) {
    ...
    const response = await $fetch<ApiListResponse<CrmProjectListItem>>('/api/world/crm/general/projects', {
      query: { page: DEFAULT_PAGE, limit: DEFAULT_LIMIT },
    })
  ...
  async function fetchTaskRequests(force = false) {
    if (!force && isFresh(taskRequestsFetchedAt.value) && taskRequests.value.length) {
