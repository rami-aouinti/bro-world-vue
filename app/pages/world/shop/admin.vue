    ...
    <v-container fluid class="pt-0">
      <v-card rounded="xl" class="pa-5 postcard-gradient-card">
        <template v-if="isRoot">
          ...
          <p class="text-medium-emphasis mb-4">Configure taxes, payment providers and order governance.</p>
          <v-row>
            <v-col cols="12" md="6">
              <v-select label="Default currency" variant="outlined" :items="['USD', 'EUR', 'GBP']" />
              <v-select label="Primary payment provider" variant="outlined" :items="['Stripe', 'PayPal', 'Adyen']" />
              <v-switch label="Enable fraud review" color="primary" inset />
            </v-col>
            <v-col cols="12" md="6">
              <v-text-field label="Tax rate (%)" type="number" variant="outlined" />
              <v-text-field label="Refund window (days)" type="number" variant="outlined" />
              <v-btn color="primary" prepend-icon="mdi-content-save-outline" block>Save Shop settings</v-btn>
            </v-col>
          </v-row>
        </template>
