...
const pageUrl = new URL('/world/shop/products', siteUrl).toString()
const seoImage = new URL('/img/platform/general/shop.png', siteUrl).toString()

useSeoMeta({
  title: t(
    'world.shop.seo.title',
    'Bro World Shop | Store, promotions, and online shopping',
  ),
  description: t(
    ...
    <v-container fluid>
      ...
      <v-card rounded="xl" class="postcard-gradient-card">
        ...
        <v-card-actions class="justify-space-between flex-wrap ga-4 px-4 py-4">
          ...
          <WorldPagination v-model="page" :length="Math.max(1, totalPages)" />
        </v-card-actions>
      </v-card>
    </v-container>
  </div>
