...
const checkoutId = computed(() => typeof route.query.checkoutId === 'string' ? route.query.checkoutId : '')
...
async function createPaymentIntent() {
  ...
  try {
    ...
    errorMessage.value = error?.statusMessage || 'Échec création payment intent (timeout/provider).'
  }
  finally {
    loading.value = false
  }
...
async function finalizeCheckout() {
  if (!checkout.value?.providerPaymentId)
    return
  ...
  try {
    ...
    errorMessage.value = error?.statusMessage || 'Le webhook provider n’a pas encore confirmé le paiement.'
  }
  finally {
    loading.value = false
  }
...
onMounted(async () => {
  if (!checkoutId.value) {
    errorMessage.value = 'checkoutId manquant. Lancez le workflow depuis /world/shop/checkout.'
    return
  }
    ...
    <v-container fluid class="pt-0">
      <v-card rounded="xl" class="pa-5">
        ...
        <p class="text-body-2 text-medium-emphasis mb-4">
          Cas gérés: timeout provider, refus paiement, double soumission (idempotency key).
        </p>
        ...
        <v-row>
          <v-col cols="12" md="6">
            <v-card variant="outlined" rounded="lg" class="pa-4 mb-4">
              ...
              />
              <v-btn color="primary" :loading="loading" :disabled="!checkoutId" @click="createPaymentIntent">
            ...
            <v-card variant="outlined" rounded="lg" class="pa-4">
              ...
              <p class="text-body-2 mb-2">Le paiement est validé uniquement après webhook provider.</p>
              <v-btn color="success" :loading="loading" :disabled="!checkout?.providerPaymentId" @click="finalizeCheckout">
                Finaliser checkout
              </v-btn>
            </v-card>
          </v-col>
          ...
          <v-col cols="12" md="6">
            <v-card variant="outlined" rounded="lg" class="pa-4">
              ...
              <p class="mb-2">Checkout ID: <strong>{{ checkout?.id || checkoutId || '—' }}</strong></p>
              <p class="mb-2">Status: <strong>{{ checkout?.status || '—' }}</strong></p>
              <p class="mb-2">Step: <strong>{{ checkout?.step || '—' }}</strong></p>
              <p class="mb-2">Total: <strong>{{ checkout?.totalAmount?.toFixed?.(2) || '0.00' }} {{ checkout?.currency || 'USD' }}</strong></p>
              <p class="mb-2">Provider payment ID: <strong>{{ checkout?.providerPaymentId || '—' }}</strong></p>
              <p class="mb-0">Dernière erreur: <strong>{{ checkout?.lastError || 'aucune' }}</strong></p>
            </v-card>

            <v-card variant="outlined" rounded="lg" class="pa-4 mt-4">
              <h3 class="text-subtitle-1 mb-3">Dernière tentative</h3>
              <p class="mb-1">Attempt ID: <strong>{{ currentAttempt?.id || checkout?.attempts?.[0]?.id || '—' }}</strong></p>
              <p class="mb-1">Statut: <strong>{{ currentAttempt?.status || checkout?.attempts?.[0]?.status || '—' }}</strong></p>
              <p class="mb-1">Provider: <strong>{{ currentAttempt?.provider || checkout?.attempts?.[0]?.provider || '—' }}</strong></p>
              <p class="mb-0">Client secret: <code>{{ currentAttempt?.clientSecret || 'non retourné' }}</code></p>
            </v-card>
          </v-col>
        </v-row>
      </v-card>
