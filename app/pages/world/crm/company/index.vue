...
const { data, pending, error, refresh } = await useFetch<
  ApiListResponse<CrmCompanyItem>
  ...
  <div>
    ...
    <v-container fluid>
      ...
      <v-alert v-else-if="error" type="error" variant="tonal">{{
        t('world.crm.company.alerts.loadListError')
      }}</v-alert>

      <v-row v-else>
        <v-col
          v-for="company in data?.items ?? []"
          :key="company.id"
          cols="12"
          md="6"
          xl="4"
        >
          <v-card rounded="xl" class="pa-4 postcard-gradient-card h-100">
            ...
            <v-btn
              ...
              @click="router.push(`/world/crm/company/${company.id}`)"
              >{{ t('world.crm.company.actions.viewDetails') }}</v-btn
            >
          </v-card>
        </v-col>
      </v-row>
    ...
    <AppModal
      ...
      :title="t('world.crm.company.modal.createTitle')"
      :max-width="720"
    >
      <v-row>
        <v-col cols="12" md="6"
          ><v-text-field
            v-model="createPayload.crmId"
            :label="t('world.crm.company.form.crmId')"
            required
        /></v-col>
        <v-col cols="12" md="6"
          ><v-text-field
