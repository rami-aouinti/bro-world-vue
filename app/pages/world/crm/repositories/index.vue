...
const privateProjectsState = useFetch<ApiListResponse<CrmProjectListItem>>('/api/crm/general/projects', {
  lazy: true,
  server: false,
  default: () => ({ items: [] }),
})

const publicProjectsState = useFetch<ApiListResponse<CrmProjectListItem>>('/api/world/crm/general/projects', {
  lazy: true,
  server: false,
  default: () => ({ items: [] }),
})
...
const showInitialSkeleton = computed(() => pendingProjects.value || (pendingRepositories.value && !hasRepositoriesData.value))
const showStaleOverlay = computed(() => pendingRepositories.value && hasRepositoriesData.value)
