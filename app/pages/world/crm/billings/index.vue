...
const { data, pending, error } = await useFetch<
  ApiListResponse<CrmBillingItem>
  ...
  <div>
    ...
    <v-container fluid>
      ...
      <v-alert v-else-if="error" type="error" variant="tonal">{{
        t('world.crm.billings.alerts.loadError')
      }}</v-alert>
      <v-row v-else>
        <v-col
          v-for="billing in data?.items ?? []"
          :key="billing.id"
          cols="12"
          md="6"
          lg="4"
        >
          <v-card rounded="xl" class="pa-4 postcard-gradient-card">
            ...
            <p class="text-body-2 mb-3">
              {{ t('world.crm.billings.list.status') }}: {{ billing.status }}
            </p>
            <v-btn
              color="primary"
              variant="tonal"
              @click="router.push(`/world/crm/billings/${billing.id}`)"
              >{{ t('world.crm.billings.actions.viewDetails') }}</v-btn
            >
          </v-card>
        </v-col>
      </v-row>
