...
useSeoMeta({
  title: t(
    'world.crm.seo.title',
    'Bro World CRM | Project, task, and customer management',
  ),
  description: t(
...
const { data: generalApplications, pending: pendingGeneralApplications } =
  useAsyncData('world-public-general-applications-crm-page', () =>
    $fetch<{ items?: CrmGeneralPublicItem[] }>(
...
function formatPathLabel(path: string) {
  return path.split('.').map(formatLabelSegment).join(' / ')
}

function toDisplayFields(
  value: unknown,
  parentPath = '',
): ConfigurationDisplayField[] {
  ...
  <div>
    ...
    <template v-else>
      <WorldModuleDrawers
        ...
        @action="
          loggedIn ? navigateTo('/platform/crm/new') : (loginDialogOpen = true)
        "
      >
        <template #right>
      ...
      <v-container fluid>
        <v-row>
          <v-col cols="12">
            <v-card
              rounded="xl"
              class="pa-6 postcard-gradient-card crm-doc-hero"
            >
              <div
              ...
              >
                <div>
                  <h1 class="text-h5 font-weight-bold mb-2">
                    {{ t('world.crm.documentation.heroTitle') }}
                  </h1>
                ...
                <v-btn
                  ...
                  @click="
                    loggedIn
                      ? navigateTo('/platform/crm/new')
                      : (loginDialogOpen = true)
                  "
                >
                  {{
                    loggedIn
                      ? t('world.crm.actions.createCrm')
                      : t('appbar.login')
                  }}
                </v-btn>
              </div>
          ...
          >
            <v-card
              rounded="xl"
              class="pa-5 postcard-gradient-card crm-doc-card h-100"
            >
              ...
              >
                {{
                  t('world.crm.documentation.openSection', {
          ...
          <v-col cols="12">
            <v-card
              rounded="xl"
              class="pa-5 postcard-gradient-card crm-doc-card"
            >
              <div class="d-flex align-center ga-2 mb-3">
                <v-icon icon="mdi-github" color="primary" />
                <h2 class="text-h6 mb-0">
                  {{ t('world.crm.documentation.githubSync.title') }}
                </h2>
              </div>
              <v-timeline
                side="end"
                density="compact"
                class="crm-sync-timeline"
              >
                <v-timeline-item
      ...
      <AppModal
        ...
        :title="
          crmGeneralApplication?.platform?.name ||
          t('world.crm.generalDrawer.platform.modalTitle', 'Platform details')
        "
        :max-width="760"
      >
        <div class="d-flex flex-column ga-4">
          <div class="text-body-2 text-medium-emphasis">
            {{
              ...
              t(
                'world.crm.generalDrawer.emptyDescription',
      ...
      <AppModal
        ...
        :title="
          selectedPlugin?.name ||
          t('world.crm.generalDrawer.plugins.modalTitle', 'Plugin details')
        "
        :max-width="760"
      >
