...
const { data, pending, error } = await useFetch<
  ApiListResponse<CrmContactItem>
  ...
  <div>
    ...
    <v-container fluid>
      <h1 class="text-h5 mb-4">
        {{ t('world.crm.contacts.list.title', 'Contacts') }}
      </h1>
      <CrmPageSkeleton v-if="pending" variant="list" :cards="6" />
      <v-alert v-else-if="error" type="error" variant="tonal">{{
        t('world.crm.contacts.alerts.loadListError')
      }}</v-alert>
      <v-row v-else>
        <v-col
          v-for="contact in data?.items ?? []"
          :key="contact.id"
          cols="12"
          md="6"
          lg="4"
        >
          <v-card rounded="xl" class="pa-4 postcard-gradient-card">
            ...
            <v-btn
              color="primary"
              variant="tonal"
              @click="router.push(`/world/crm/contacts/${contact.id}`)"
              >{{ t('world.crm.contacts.actions.viewDetails') }}</v-btn
            >
          </v-card>
        </v-col>
      </v-row>
