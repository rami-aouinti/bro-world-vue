...
const learningItems = computed(() => {
  ...
  return isRoot.value
    ? [...base, { title: 'Admin', to: '/world/learning/admin', icon: 'mdi-shield-crown-outline' }]
    : base
})
</script>
...
<template>
  <v-container fluid>
    <v-card rounded="xl" class="pa-3">
      <v-card-title class="text-h5">Learning</v-card-title>
      <v-row class="mt-2">
        <v-col v-for="item in learningItems" :key="item.to" cols="12" sm="6">
