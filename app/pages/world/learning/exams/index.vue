...
function normalizeName(entry: Record<string, unknown>) {
  const fullName =
    `${String(entry.firstName ?? '')} ${String(entry.lastName ?? '')}`.trim()
...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    if (
      selectedClass.value &&
      String(item.className ?? '').trim() !== selectedClass.value
    ) {
      return false
    }
    ...
    if (
      ...
      String(item.courseId ?? '').trim() !== selectedCourse.value
    ) {
      return false
    }
    ...
    if (
      selectedType.value &&
      String(item.type ?? '').trim() !== selectedType.value
    ) {
      return false
    }
