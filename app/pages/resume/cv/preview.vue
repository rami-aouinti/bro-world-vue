...
const activeColors = computed(() => {
  ...
  if (selected && selected.value !== 'template') return { ...palette, primary: selected.primary, secondary: selected.secondary ?? selected.dark, text: selected.text, muted: selected.tertiary, pageBackground: selected.quaternary }
  return palette
})
...
const normalizedStructure = computed(() => {
  ...
  if (structure === 'structure-1' || structure === 'structure-2') return structure
  const layout = String(activeTemplate.value?.layout || '')
  return layout === 'no-aside-split' ? 'structure-2' : 'structure-1'
})

const isMainStructureLayout = computed(() => ['aside', 'no-aside', 'no-aside-split'].includes(String(activeTemplate.value?.layout || '')))
...
v-if="!isCaptureMode"
      ...
      <div class="py-8">
        ...
        >
          ...
          <template #aside>
            <div v-if="isSideContentLayout && normalizedStructure === 'structure-1'" :class="['cv-aside-sections', { 'cv-aside-sections--full': ['aside-full-left', 'aside-full-right'].includes(String(activeTemplate?.layout || '')) }]">
            ...
            <div v-else-if="isSideContentLayout && normalizedStructure === 'structure-2'" :class="['cv-aside-sections', { 'cv-aside-sections--full': ['aside-full-left', 'aside-full-right'].includes(String(activeTemplate?.layout || '')) }]">
          ...
          <template #content>
            <div v-if="isSideContentLayout && normalizedStructure === 'structure-1'" class="cv-sections-list">
            ...
            <div v-else-if="isSideContentLayout && normalizedStructure === 'structure-2'" class="cv-sections-structure-2">
            ...
            <div v-else-if="isMainStructureLayout && normalizedStructure === 'structure-1'" class="cv-sections-list">
            ...
            <div v-else-if="isMainStructureLayout && normalizedStructure === 'structure-2'" class="cv-sections-structure-2">
...
.cv-header-editor--link-value{opacity:0;max-width:0;overflow:hidden;transition:all .2s ease}
.cv-contact-item:hover .cv-header-editor--link-value{opacity:1;max-width:320px}
