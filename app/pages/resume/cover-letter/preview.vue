...
.capture-cover-letter{position:relative;overflow:hidden;box-sizing:border-box;width:min(100%, 850px);min-height:1123px;padding:56px 64px 66px;background:var(--cp-bg);color:var(--cp-text)}
.capture-cover-letter::after{content:'';position:absolute;left:0;right:0;bottom:0;height:10px;background:#fff}
.hero{border-left:var(--bar-primary-width) solid var(--cp-primary);padding-left:24px;padding-top:6px;margin-bottom:42px;border-radius:var(--bar-radius);position:relative;min-height:140px}.meta-top-right{position:absolute;top:0;right:0;display:flex;flex-direction:column;align-items:flex-end;gap:2px;text-align:right}.meta-top-right--layout-right{right:auto;left:0;text-align:left;align-items:flex-start}.hero-row{display:flex;flex-direction:column;align-items:flex-start;gap:6px;padding-top:8px}.hero-avatar{align-self:flex-start}.hero-avatar--right{align-self:flex-end}.hero--photo-right{padding-top:8px}.hero--ribbon{padding-top:16px;padding-bottom:12px}
