...
watch(activeTemplate, (tpl) => { editableDecorObjects.value = JSON.parse(JSON.stringify(tpl?.decor?.objects || [])); const items=(tpl as any)?.items||{}; for (const key of ['date','address']) { const b=items[key]?.size; if (b) letterElementStyles[key].size=Math.round((b.min+b.max)/2); if (items[key]?.colors?.[0]) letterElementStyles[key].color=items[key].colors[0]; if (items[key]?.styles?.[0]) letterElementStyles[key].weight=fontWeightMap[items[key].styles[0]]||'400' } }, { immediate: true })
...
<style scoped>
.capture-cover-letter{position:relative;overflow:hidden;width:850px;min-height:1123px;padding:80px;background:var(--cp-bg);color:var(--cp-text)}.meta-top-right{display:flex;flex-direction:column;align-items:flex-end;gap:10px;margin:0 0 52px auto;text-align:right;width:fit-content;max-width:100%}.letter-body{display:flex;flex-direction:column;align-items:flex-start;gap:16px}.letter-body :deep(p){margin:0;line-height:1.45}.letter-body :deep(p:first-child){font-size:56px;font-weight:700;color:var(--cp-primary);margin-bottom:8px}.letter-body :deep(p:nth-child(2)){font-size:48px;font-weight:700;color:var(--cp-text);margin-bottom:10px}.letter-body :deep(p:nth-child(3)),.letter-body :deep(p:nth-child(4)){max-width:92%;font-size:var(--body-size);color:var(--body-color)}.signature-rule{width:130px;height:3px;background:var(--cp-secondary);margin:14px 0 6px}.decor-object{position:absolute;pointer-events:none;background:color-mix(in srgb,var(--cp-primary) 35%,transparent)}.decor-circle{border-radius:999px}.decor-ring{border-radius:999px;background:transparent;border:3px solid color-mix(in srgb,var(--cp-secondary) 55%,transparent)}.decor-blob{border-radius:40% 60% 55% 45% / 50% 35% 65% 50%}.preview-toolbar-wrap{position:sticky;top:76px;z-index:20;display:flex;justify-content:center}.preview-toolbar-row{display:flex;flex-wrap:wrap;gap:8px;padding:10px 12px;border:1px solid rgba(148,163,184,.35);border-radius:999px;background: rgba(var(--v-theme-primary))}.signature-footer{margin-top:32px}.signature-image{height:68px;object-fit:contain}.template-menu-card{width:min(92vw,980px);max-height:min(76vh,760px);overflow:auto}.template-menu-grid{display:grid;gap:10px;grid-template-columns:repeat(4,minmax(0,1fr));padding:12px}.template-menu-item{cursor:pointer;border-radius:12px;overflow:hidden;border:1px solid rgba(15,23,42,.08)}.template-menu-item--active{border-color:rgb(var(--v-theme-primary));box-shadow:0 0 0 1px rgb(var(--v-theme-primary))}.template-menu-item :deep(.v-img){border-bottom:1px solid rgba(15,23,42,.08)}@media (max-width:1100px){.template-menu-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
</style>
<style scoped>
