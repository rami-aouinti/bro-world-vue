...
<template>
  <v-card
    class="h-100 football-surface football-surface--glow football-interactive-card"
    variant="outlined"
  >
    <v-card-title>{{
      t('pages.applications.basketball.playerDetails')
    }}</v-card-title>
    <v-divider />
    <v-card-text>
      <template v-if="loadingOverride || playerState === 'loading'">
        ...
        />
        <span>{{
          t('pages.applications.basketball.loadingPlayerDetails')
        }}</span>
      </template>
      ...
      >
        {{
          playerError ||
          t('pages.applications.basketball.unableToLoadPlayerDetails')
        }}
      </v-alert>
        ...
        <div class="d-flex flex-wrap align-center ga-4 mb-5">
          <v-avatar size="84" class="player-avatar">
            <v-img
              ...
              :src="profile.photo"
              :alt="
                profile?.name || t('pages.applications.basketball.playerPhoto')
              "
              cover
            />
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="text-caption text-medium-emphasis mt-1">
              {{
                profile?.height ||
                t('pages.applications.basketball.heightNotAvailable')
              }}
              ·
              {{
                profile?.weight ||
                t('pages.applications.basketball.weightNotAvailable')
              }}
            </div>
            <v-chip
        ...
        <v-card variant="outlined" class="mb-5">
          <v-card-text class="d-flex align-center ga-3">
            <sports-basketball-team-avatar
              :team-name="
                team?.name || t('pages.applications.football.misc.unknownTeam')
              "
              :logo="team?.logo || null"
              :size="36"
            />
            <div>
              <div class="text-subtitle-2">
                {{ t('pages.applications.football.stats.team') }}
              </div>
              <div class="text-body-2">
                {{
                  team?.name ||
                  t('pages.applications.football.misc.unknownTeam')
                }}
              </div>
            </div>
          </v-card-text>
