...
<template>
  ...
  >
    ...
    <section>
      ...
      <p class="cover-letter-split__sign">{{ props.model.fullName }}</p>
    </section>
  </article>
</template>
...
.cover-letter-split aside {
  background: color-mix(in srgb, var(--cl-soft) 72%, var(--cl-page));
  padding: 30px 24px;
  display: grid;
  align-content: start;
  gap: 8px;
...
.cover-letter-split h2 {
  color: var(--cl-page);
  font-size: 1.3rem;
  margin-bottom: 4px;
}
.cover-letter-split section {
  padding: 34px 36px;
  display: grid;
  gap: 12px;
  line-height: 1.68;
  background: linear-gradient(
    180deg,
    transparent 0 82%,
    color-mix(in srgb, var(--cl-soft) 30%, transparent)
  );
}
.cover-letter-split section p:first-child {
  text-transform: uppercase;
  letter-spacing: 0.07em;
  font-size: 0.78rem;
}
.cover-letter-split__sign {
  margin-top: 8px;
  font-weight: 600;
  letter-spacing: 0.04em;
  color: color-mix(in srgb, var(--cl-accent) 76%, black);
}
@media (max-width: 960px) {
  ...
  .cover-letter-split aside {
    border-right: 0;
    border-bottom: var(--cl-divider-width) var(--cl-divider-style)
      color-mix(in srgb, var(--cl-accent) 20%, transparent);
  }
}
</style>
