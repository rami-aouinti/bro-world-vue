...
<template>
  <div class="executive-template cv-template-base text-dark">
    <aside class="executive-sidebar cv-sidebar-surface cv-gradient-sidebar">
      ...
      <h1>
        <span class="editable-text text-dark" :contenteditable="editable" @input="event => updateText('firstName', (event.target as HTMLElement).innerText)">{{ resume.firstName }}</span><br>
      ...
      <p class="title editable-text text-dark" :contenteditable="editable" @input="event => updateText('role', (event.target as HTMLElement).innerText)">{{ resume.role }}</p>
...
h2 { color: var(--cv-title); margin-bottom: 9px; font-size: 1rem; text-transform: uppercase; letter-spacing: .08em; }
...
.entry-header p { color: var(--cv-secondary); /* Intentional neutral gray metadata. */ font-size: .8rem; }
