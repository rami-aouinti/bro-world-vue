...
import { getGeneratedTemplateSectionForm } from '~/utils/generatedTemplateNormalizer'
...
const autoVariants = computed<VariantOption[]>(() => {
  ...
  GENERATED_RESUME_TEMPLATES.forEach((template) => {
    aliases.forEach((alias) => {
      const variant = getGeneratedTemplateSectionForm(
        template.sections,
        alias,
        '',
      )
...
const SECTION_KEY_TO_RENDERER_KEY: Record<
  string,
  keyof typeof sectionRendererRegistry
> = {
...
const variantOptions = computed(() =>
  props.variants.length
    ? props.variants
    : registryVariants.value.length
      ? registryVariants.value
      : autoVariants.value,
)
const addFields = computed(
  () =>
    SECTION_ADD_FIELDS[normalizedSectionKey(props.sectionKey)] || [
...
watch(
  ...
  (value) => {
    selectedVariant.value = value || variantOptions.value[0]?.value || ''
  },
  { immediate: true },
)
...
const selectedVariantForUi = computed(() =>
  selectedVariant.value === 'two-column'
    ? 'two-columns'
...
.resume-hover-section__select-wrap {
  min-width: 170px;
}
.resume-hover-section__select :deep(.v-field) {
  min-height: 32px;
}
