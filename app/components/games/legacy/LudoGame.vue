...
const players = ref<PlayerState[]>([
  ...
  {
    id: 'blue',
    name: t('gameComponents.ludo.players.blue'),
    isAi: props.selectedPlayMode === 'ai',
  },
])
...
const remainingToGoal = (player: PlayerColor) =>
  pawns.value.filter(
    (pawn) => pawn.player === player && pawn.steps < PATH.length - 1,
  ).length
...
const runAiTurn = () => {
  ...
  const firstPlayable = pawns.value.find((pawn) =>
    playablePawnIds.value.has(pawn.id),
  )
  if (firstPlayable) {
...
const panelState = computed<GameAsidePanelState>(() => ({
  ...
  highlights: [
    ...
    hasRolled.value && diceValue.value
      ? t('gameComponents.ludo.highlights.diceCurrent', {
          dice: diceValue.value,
        })
...
<template>
  <GameTableScaffold class="game-shell-unified" surface-variant="flat">
    <template #surface>
      <div class="ludo-board-wrap">
        <div class="ludo-board">
          <div v-for="row in 11" :key="`line-${row}`" class="ludo-row">
            <button
              v-for="col in 11"
    ...
    <div class="d-flex align-center justify-space-between flex-wrap ga-2 mb-1">
      <h3 class="game-shell-title mb-0">
        {{ t('gameComponents.ludo.title') }}
      </h3>
            ...
            {{
              pawn.steps < 0
                ...
                : pawn.steps >= PATH.length - 1
                  ...
                  : t('gameComponents.ludo.pawn.cell', {
                      count: pawn.steps + 1,
                    })
            }}
          </span>
...
.pawn-selector--playable {
  box-shadow: inset 0 0 0 2px
    color-mix(in srgb, rgb(var(--v-theme-primary)) 38%, transparent);
}
.pawn-selector--selected {
