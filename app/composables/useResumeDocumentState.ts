...
export function normalizeDecorativeShape(shape: unknown, defaults: ResumeDocumentModel['style']['decorativeShapeA'], allowedTypes: readonly ResumeDocumentModel['style']['decorativeShapeA']['type'][]): ResumeDocumentModel['style']['decorativeShapeA'] {
  if (!isRecord(shape)) return { ...defaults }
  return { enabled: typeof shape.enabled === 'boolean' ? shape.enabled : defaults.enabled, type: normalizeEnum(shape.type, allowedTypes, defaults.type), width: clampNumber(shape.width, 30, 360, defaults.width), height: clampNumber(shape.height, 30, 360, defaults.height), size: clampNumber(shape.size, 30, 360, defaults.size), color: typeof shape.color === 'string' ? shape.color : defaults.color, opacity: clampNumber(shape.opacity, 0.05, 1, defaults.opacity), x: clampNumber(shape.x, 0, 100, defaults.x), y: clampNumber(shape.y, 0, 100, defaults.y), rotation: clampNumber(shape.rotation, -180, 180, defaults.rotation) }
}
...
export function normalizeSectionOrder(sectionOrder: unknown): ResumeSectionLayoutModel[] {
  ...
  return DEFAULT_SECTION_ORDER.map((fallback) => {
    ...
    return { key: fallback.key, variant: typeof parsed?.variant === 'string' ? parsed.variant : fallback.variant, region: parsed?.region === 'main' || parsed?.region === 'aside' ? parsed.region : fallback.region, order: typeof parsed?.order === 'number' ? parsed.order : fallback.order }
  })
}

export function normalizeStyle(styleSource: unknown, defaultLayoutMode: ResumeDocumentModel['style']['layoutMode']): ResumeDocumentModel['style'] {
...
export function useResumeDocumentState(presetId: Ref<string>) {
  const state = useState<ResumeDocumentCustomizationState>('resume-document-customization-state', () => ({ customization: normalizeModel(null, presetId.value) }))
  function hydrateFromStorage() { if (!import.meta.client) return; const raw = localStorage.getItem(STORAGE_KEY); if (!raw) return; try { const parsed = JSON.parse(raw); state.value.customization = normalizeModel(parsed, presetId.value) } catch { localStorage.removeItem(STORAGE_KEY) } }
  function migrateLegacyStorage(layoutSettingsRaw: string | null, sectionLayoutRaw: string | null) { if (!import.meta.client) return; const current = state.value.customization; let nextStyle = { ...current.style }; let nextSectionOrder = current.sectionOrder.map(entry => ({ ...entry })); if (layoutSettingsRaw) { try { const parsed = JSON.parse(layoutSettingsRaw) as Partial<{ lineDensity: 'compact' | 'comfortable' }>; if (parsed.lineDensity) nextStyle = { ...nextStyle, density: parsed.lineDensity } } catch {} } if (sectionLayoutRaw) { try { const parsed = JSON.parse(sectionLayoutRaw); nextSectionOrder = normalizeSectionOrder(parsed) } catch {} } state.value.customization = { ...current, style: nextStyle, sectionOrder: nextSectionOrder } }
  function persist() { if (!import.meta.client) return; localStorage.setItem(STORAGE_KEY, JSON.stringify(state.value.customization)) }
  watch(presetId, (nextPresetId) => { state.value.customization = { ...state.value.customization, presetId: resolveSoclePresetById(nextPresetId).id } })
  return { storageKey: STORAGE_KEY, defaultSectionOrder: DEFAULT_SECTION_ORDER, state, hydrateFromStorage, migrateLegacyStorage, persist }
}
