...
export function normalizeDecorativeShape(
  ...
  return {
    enabled:
      typeof shape.enabled === 'boolean' ? shape.enabled : defaults.enabled,
...
export function normalizeSectionOrder(
  ...
  if (!Array.isArray(sectionOrder))
    return DEFAULT_SECTION_ORDER.map((entry) => ({ ...entry }))
  ...
  return DEFAULT_SECTION_ORDER.map((fallback) => {
    ...
    return {
      key: fallback.key,
      variant:
        typeof parsed?.variant === 'string' ? parsed.variant : fallback.variant,
      region:
        parsed?.region === 'main' || parsed?.region === 'aside'
          ? parsed.region
          : fallback.region,
...
export function normalizeStyle(
  ...
  return {
    palette:
      typeof style.palette === 'string' ? style.palette : defaults.palette,
    pageBackground: normalizeEnum(
      style.pageBackground,
      ['white', 'sky-light', 'pearl-light', 'ivory-light'] as const,
    ...
    radius:
      (style.radius as ResumeDocumentModel['style']['radius']) ??
      defaults.radius,
    typography:
      (style.typography as ResumeDocumentModel['style']['typography']) ??
      defaults.typography,
    photoPosition: normalizeEnum(
      style.photoPosition,
      ['left', 'right'] as const,
      defaults.photoPosition,
    ),
    asideWidth: clampNumber(
      style.asideWidth,
      220,
      380,
      defaults.asideWidth,
    ...
    asideHeight: clampNumber(
      style.asideHeight,
      60,
      100,
      defaults.asideHeight ?? 100,
      true,
    ),
    showSectionIcons:
      typeof style.showSectionIcons === 'boolean'
        ? style.showSectionIcons
        : defaults.showSectionIcons,
    showContactIcons:
      typeof style.showContactIcons === 'boolean'
        ? style.showContactIcons
        : defaults.showContactIcons,
    sectionIconStyle: normalizeEnum(
      style.sectionIconStyle,
      ['outline', 'filled', 'rounded'] as const,
      defaults.sectionIconStyle,
    ),
    iconSize: normalizeEnum(
      style.iconSize,
      ['s', 'm', 'l'] as const,
      defaults.iconSize,
    ),
    iconColor: normalizeEnum(
      style.iconColor,
    ...
    layoutMode: normalizeEnum(
      style.layoutMode,
      ['aside-left', 'aside-right', 'no-aside'] as const,
      defaults.layoutMode,
    ),
    decorativeShapeA: normalizeDecorativeShape(
      style.decorativeShapeA,
      defaults.decorativeShapeA,
      ['circle', 'square', 'ring', 'bar'] as const,
    ),
    decorativeShapeB: normalizeDecorativeShape(
      style.decorativeShapeB,
      defaults.decorativeShapeB,
      ['circle', 'square', 'ring', 'bar'] as const,
    ),
  }
}
...
export function normalizeModel(
  ...
  if (!isRecord(source)) {
    return {
      ...
      style: normalizeStyle(null, fallbackPreset.layoutMode),
    }
  }
  ...
  const template = {
    structureId:
      typeof sourceTemplate.structureId === 'string'
        ? sourceTemplate.structureId
        : fallbackTemplate.structureId,
    layoutId:
      typeof sourceTemplate.layoutId === 'string'
        ? sourceTemplate.layoutId
        : fallbackTemplate.layoutId,
    skinId:
      typeof sourceTemplate.skinId === 'string'
        ? sourceTemplate.skinId
        : fallbackTemplate.skinId,
  }
...
export function useResumeDocumentState(presetId: Ref<string>) {
  const state = useState<ResumeDocumentCustomizationState>(
    'resume-document-customization-state',
    () => ({ customization: normalizeModel(null, presetId.value) }),
  )
  function hydrateFromStorage() {
    ...
    try {
      ...
      state.value.customization = normalizeModel(parsed, presetId.value)
    } catch {
      localStorage.removeItem(STORAGE_KEY)
    }
  }
  function migrateLegacyStorage(
    layoutSettingsRaw: string | null,
    sectionLayoutRaw: string | null,
  ) {
    if (!import.meta.client) return
    ...
    if (layoutSettingsRaw) {
      try {
        const parsed = JSON.parse(layoutSettingsRaw) as Partial<{
  ...
  watch(presetId, (nextPresetId) => {
    state.value.customization = {
      ...state.value.customization,
      presetId: resolveSoclePresetById(nextPresetId).id,
    }
  })
  return {
